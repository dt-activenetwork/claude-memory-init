...
**核心变化**:
1. **目录结构**: `claude/` → `.agent/`
2. **入口文件**: `CLAUDE.md` → `AGENT.md`
3. **数据格式**: JSON/YAML → **TOON (一等公民)**
4. **架构**: CLI-Prompt 分离 → **插件-Prompt 一体化**

**新增组件**:
- `src/constants.ts` - 全局常量（DEFAULT_AGENT_DIR = '.agent'）
- `src/utils/toon-utils.ts` - TOON 工具函数
- `src/core/agent-assembler.ts` - AGENT.md 组装器
- `templates/AGENT.md.template` - 占位符模板
- `src/plugins/index.ts` - 插件注册表
...
**实现文件**:
- `src/plugins/git/index.ts` (350+ 行)
- CLI 集成 (`initPluginBased()`)
- 测试脚本 (`test-git-plugin.ts`, `test-e2e-simple.ts`)
...
config:
  ...
  ignore_patterns[2]: .agent/temp/,.agent/.cache/
...
See `.agent/git/rules.md` for complete guidelines.
```
...
1. 查看本记忆文件了解当前进度
2. 选择方向：
   - 继续插件开发（Phase 4.2 Prompt Presets, Phase 4.3 Memory System）
   - 或完善现有系统（测试、文档、边界情况）
3. 继续推进 v2.0 重构

**当前状态** (2025-01-19):
- ✅ 核心架构完成（插件系统 + TOON + .agent/）
- ✅ 2 个插件完成（system-detector + git）
- ✅ CLI 集成完成
- ✅ 133/133 测试通过
- ✅ E2E 验证通过
- 🎯 架构已稳定，可以继续扩展
